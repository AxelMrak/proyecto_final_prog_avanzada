...
\begin{figure}[H]
    ...
    \caption{Menú principal y listado de alumnos.}
    \label{fig:app1}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Inscripción de un alumno a una materia: selección por lista
    numerada.}
    \label{fig:app3}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Carga de nota: el docente selecciona alumno y materia de
    listas desplegables e ingresa la nota.}
    \label{fig:app5}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{../Recursos/estructura.png}
    \caption{Diagrama de la estructura de capas y paquetes del proyecto.}
...
\begin{table}[H]
    ...
    \begin{tabularx}{0.92\textwidth}{l X}
        ...
        \midrule
        \texttt{demo.mov}
            & Video: demostración de la aplicación funcionando. \\
        \texttt{demo-codigo-y-estructura.mov}
            & Video: recorrido por el código fuente y la arquitectura. \\
        \texttt{estructura.png}
            & Diagrama de la arquitectura en capas. \\
        \texttt{interaccion-app1.png}
            & Captura: menú principal y listado de alumnos. \\
        \texttt{interaccion-app2.png}
            & Captura: alta de un nuevo alumno. \\
        \texttt{interaccion-app3.png}
            & Captura: inscripción de alumno a materia (selección por lista). \\
        \texttt{interaccion-app4.png}
            & Captura: confirmación de inscripción registrada. \\
        \texttt{interaccion-app5.png}
            & Captura: carga de nota (selección de alumno y materia). \\
        \texttt{interaccion-app6.png}
            & Captura: confirmación de nota cargada y menú de docentes. \\
        \bottomrule
    \end{tabularx}
    \caption{Listado del material en \texttt{Recursos/}.}
\end{table}
...
\begin{itemize}
    \item Se obtuvo un \textbf{sistema de gestión universitaria funcional}
        en Java~17, con persistencia en SQLite, menú de consola interactivo
        y validación por campo.
    \item La \textbf{arquitectura en cuatro capas} demostró ser efectiva:
        el dominio está completamente aislado de JDBC y de la terminal; los
        servicios de aplicación contienen las reglas de negocio; la
        infraestructura es intercambiable sin tocar el dominio.
    \item La aplicación deliberada de los principios \textbf{SOLID} --- en
        particular SRP, ISP y DIP --- produjo un código con bajo
        acoplamiento y alta cohesión, donde cada clase tiene un propósito
        claro.
    \item El \textbf{esquema de base de datos normalizado} (3NF) con
        tablas de lookup, restricciones CHECK e índices sobre claves
        foráneas garantiza integridad referencial y evita datos
        inconsistentes.
    \item La \textbf{experiencia de usuario} se mejoró significativamente
        con listas numeradas desplegables para seleccionar alumnos y
        materias, eliminando la necesidad de memorizar legajos o códigos.
\end{itemize}
